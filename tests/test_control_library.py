...
async def q_songs(session, uid, since=0, playlist_sync_id=None):
    ...
    return [Row(videoId='s2', title='Two', uploader='U', duration=100,
                thumbnail=None, position=2, deleted=False),
            Row(videoId='s1', title='One', uploader='U', duration=90,
                thumbnail=None, position=1, deleted=False)]
...
def load(factory='ok'):
    ...
    for n in tree.body:
        if isinstance(n, (ast.FunctionDef, ast.AsyncFunctionDef)) \
            ...
            t = n.targets[0]
            if isinstance(t, ast.Name) and t.id == '_LIBRARY_CAP':
                chunks.append('\n'.join(lines[n.lineno - 1:n.end_lineno]))
    assert len(chunks) == 2, 'handler or cap not found'
    # _LIBRARY_CAP is defined AFTER the handler in main.py (fine at runtime,
    # resolved at call time) — exec the cap first here for the same effect.
    chunks.reverse()
...
async def main():
    ...
    await settle()
    m = sent[-1][1]
    check("deleted playlist hidden, live one present",
          [i["syncId"] for i in m["items"]] == ['p1'])
