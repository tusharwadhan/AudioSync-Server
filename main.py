...
@api.get("/debug/related-probe")
async def debug_related_probe(video_id: str = "kJQP7kiw5Fk"):
    """Which innertube clients can this HOST still reach?

    Exists because the failure is invisible from a development machine: every
    one of these succeeds from a residential IP, and the 403 only happens
    from Render. Without this, each candidate client costs a full deploy to
    test, one at a time. This tries them all in one request and reports what
    the box can actually do.

    Safe to leave in — it downloads nothing and returns status codes, not
    content. Delete once the source is settled.
    """
    clients = [
        ("WEB_REMIX", "https://music.youtube.com", {
            "clientName": "WEB_REMIX", "clientVersion": _YTM_WEB_VERSION, "hl": "en"}),
        ("ANDROID_MUSIC", "https://music.youtube.com", {
            "clientName": "ANDROID_MUSIC", "clientVersion": _YTM_MOBILE_VERSION,
            "androidSdkVersion": 30, "hl": "en"}),
        ("IOS_MUSIC", "https://music.youtube.com", {
            "clientName": "IOS_MUSIC", "clientVersion": "7.21.2", "hl": "en"}),
        ("WEB", "https://www.youtube.com", {
            "clientName": "WEB", "clientVersion": "2.20260801.00.00", "hl": "en"}),
        ("ANDROID", "https://www.youtube.com", {
            "clientName": "ANDROID", "clientVersion": "20.10.38",
            "androidSdkVersion": 30, "hl": "en"}),
        ("TVHTML5", "https://www.youtube.com", {
            "clientName": "TVHTML5", "clientVersion": "7.20250101.00.00", "hl": "en"}),
        ("ANDROID_VR", "https://www.youtube.com", {
            "clientName": "ANDROID_VR", "clientVersion": "1.65.10",
            "androidSdkVersion": 32, "hl": "en"}),
    ]
    out = []
    async with httpx.AsyncClient(timeout=httpx.Timeout(12.0, connect=5.0)) as c:
        for name, origin, client in clients:
            row = {"client": name}
            try:
                r = await c.post(
                    f"{origin}/youtubei/v1/next?alt=json",
                    json={
                        "videoId": video_id,
                        "playlistId": f"RD{video_id}",
                        "isAudioOnly": True,
                ...
                if r.status_code == 200:
                    try:
                        row["queue"] = len(list(_walk_queue(r.json())))
                    except Exception as e:
                        row["queue"] = f"parse: {type(e).__name__}"
            except Exception as e:
                row["status"] = f"{type(e).__name__}: {str(e)[:80]}"
            out.append(row)
    return {"videoId": video_id, "results": out}
